...
def train(model, optimizer, criterion, train_loader, test_x, test_y):
    ...
    for epoch in range(MAX_EPOCH):
        ...
        print("epoch: {}, train_loss {:.3f}, test_loss {:.3f} precision {:.3f} using time {:.3f}".format(
            epoch, train_loss_list[-1], test_loss_list[-1], precision_list[-1], time.time() -
            start_time
        ))
...
if __name__ == '__main__':
    ...
    pre_ax.plot(iteration_num_list, precision_list,
                label=label, linewidth=0.5)  # 绘制
    train_loss_ax.plot(iteration_num_list, train_loss_list,
                       label=label, linewidth=0.5)
    test_loss_ax.plot(iteration_num_list, test_loss_list,
                      label=label, linewidth=0.5)
