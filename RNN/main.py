...
def process_poems(file_name):
    ...
    word_int_map = {word: i for (i, word) in enumerate(words_set)}
    int_word_map = {i: word for (i, word) in enumerate(words_set)}
    poems_vector = []
    for poem in poems:
        index = [word_int_map[w] for w in poem]
        poems_vector.append(index)
    ####################################################
...
def generate_batch(batch_size, poems_vec):
    ...
    for i in range(n_chunk):
        ...
        x_data = [poem_vec[:-1]
                  for poem_vec in poems_vec[start_index:end_index]]
        y_data = [poem_vec[1:]
                  for poem_vec in poems_vec[start_index:end_index]]
...
def run_training():
    ...
    word_embedding = rnn.word_embedding(vocab_length=len(
        word_to_int) + 1, embedding_dim=WORD_EMBEDDING_DIM)
    # 建立RNN模型,之前建立的word embedding层作为它的一部分
    # print(word_embedding(1))
    # return
    rnn_model = rnn.RNN_model(vocab_len=len(word_to_int) + 1,
                              word_embedding=word_embedding,
                              embedding_dim=WORD_EMBEDDING_DIM,
                              lstm_hidden_dim=LSTM_HIDDEN_DIM)
    optimizer = optim.Adam(rnn_model.parameters(), lr=3e-3)
    Criterion = nn.CrossEntropyLoss()
    # 如果你想读取之前训练得到的模型并接着进行训练，请去掉这一行前面的#号
...
def gen_poem(begin_word):
    ...
    poems_vector, word_int_map, int_word_map = process_poems(
        './subset_poems.txt')
    word_embedding = rnn.word_embedding(vocab_length=len(
        word_int_map) + 1, embedding_dim=WORD_EMBEDDING_DIM)
    rnn_model = rnn.RNN_model(vocab_len=len(word_int_map) + 1,
                              word_embedding=word_embedding,
                              embedding_dim=WORD_EMBEDDING_DIM,
                              lstm_hidden_dim=LSTM_HIDDEN_DIM)
...
if __name__ == '__main__':
    # 开始训练
    # process_poems('./subset_poems.txt')
    # run_training()  # 如果不是训练阶段 ，请注销这一行。 因为网络训练时间很长。

    # #开始测试，生成诗句
    print(gen_poem("日照香炉生"))  # 这里的字可以自己指定，但是必须存在于word_embedding中
